    ...
    if (isLoading) {return (<div>로딩중입니다.</div>);}
        ...
        return (
            <Wrapper title={insurance.name} underline={false}>
                <Form form={form} labelCol={{span: 10}} wrapperCol={{span: 14}} layout="vertical" size={"large"}
                    ...
                    <SelectOptions onChangeMethod={handleChange} selectName='category' selectValue={updateData.category}
                                   selectRequired={true}
                                   selectLabel={'상품 항목'} selectPlaceholder={'상품의 종류를 선택하세요'}
                                   optionList={insuranceCategory}/>

                    <Row>
                        <Col span={7}>
                            <Form.Item wrapperCol={12} label="가입 연령대">
                                <InputNumber style={{display: 'inline-block', width: '45%', marginInlineEnd: '4px'}}
                                             placeholder="가입 최저 연령"
                                             min={0} max={100.00} name="startAge" value={updateData.conditions?.startAge}
                                             onChange={(val) => {
                                                 handleChange({target: {name: 'conditions', value: {startAge: val}}});
                                             }}/>
                                ...
                                <InputNumber style={{display: 'inline-block', width: '45%'}} placeholder="가입 최고 연령"
                                             ...
                                             onChange={(val) => {
                                                 handleChange({target: {name: 'conditions', value: {endAge: val}}});
                                             }}/>
                            </Form.Item>
                        </Col>
                        <Col span={7}>
                            <Form.Item wrapperCol={12} name={"rating"} label="최소 신용등급">
                                <InputNumber style={{display: 'inline-block', width: '100%'}}
                                             ...
                                             onChange={(val) => {
                                                 handleChange({target: {name: 'conditions', value: {rating: val}}});
                                             }}/>
                            </Form.Item>
                        </Col>
                    </Row>

                    <Form.Item rules={[{required: true, message: '보험의 개괄적인 설명을 입력해주세요'}]} name={"description"}
                               label="보험상품 개요">
